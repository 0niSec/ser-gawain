...
class Crafting(commands.GroupCog):
    ...
    async def request(
        ...
        self.cursor.execute("SELECT * FROM users WHERE user_id = ?", (requestor_id,))
        ...
        request_embed.add_field(
            ...
            value=f"**ID:** {request_id}\n**Item:** {item}\n**Amount:** {amount}\n**Has Materials:** {has_materials}\n**Trade Skill:** {skill.value}\n**Level Required:** {level_required}",
        )

        request_view = RequestView(request_id, item)

        await interaction.response.send_message(embed=request_embed, view=request_view)
    ...
    async def status(self, interaction: discord.Interaction, request_id: str):
        ...
        await interaction.response.send_message(
            f"Checking the status of crafting request {request_id}!"
        )
        ...
        if request is None:
            await interaction.followup.send(f"Crafting request {request_id} not found.")
            return
